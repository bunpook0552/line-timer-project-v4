...
const firebaseAppRoot = !getApps().length ? initializeApp(firebaseConfigRoot) : getApp();
const dbRoot = getFirestore(firebaseAppRoot);

export default async function handlerRoot(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', ['POST']);
    return res.status(405).end(`Method ${req.method} Not Allowed`);
  }
  ...
  try {
    ...
    const storesCollectionRef = collection(dbRoot, 'stores');
    const docRef = await addDoc(storesCollectionRef, {
      name: storeName,
      location,
      createdAt: new Date(),
    ...
    console.error('Error creating store (root):', error);
    res.status(500).json({ message: 'เกิดข้อผิดพลาดในการสร้างร้านค้า', error: error.message });
  }
}
