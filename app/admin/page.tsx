...
export default function AdminPage() {
  ...
  const STORE_ID = 'laundry_1'; // <--- กำหนด ID ร้านค้าของคุณที่นี่
  ...
  const fetchActiveTimers = async () => {
    ...
    try {
      ...
      timerList.sort((a, b) => {
        const dateA = new Date(a.end_time.seconds * 1000);
        const dateB = new Date(b.end_time.seconds * 1000);
        return dateA.getTime() - dateB.getTime();
      });
      setActiveTimers(timerList);
    } catch (err: unknown) {
      console.error("Error fetching active timers:", err);
      if (typeof err === 'object' && err !== null && 'code' in err && (err as any).code === 'failed-precondition') {
  ...
  const handleSaveNewMachine = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingMachine = await getDocs(q);
  ...
  const handleSaveMessageClick = async (templateDocId: string) => {
    try {
      ...
      await updateDoc(templateRef, { text: editMessageFormData });
        ...
        <div className="card">
                    ...
                    {machines.map(machine => (
                      <tr key={machine.id} style={{ borderBottom: '1px dashed #eee' }}>
                        ...
                        <td style={{ padding: '8px', textAlign: 'right' }}>
                          {editingMachineId === machine.id ? (
                            <>
                              <button className="line-button" style={{ backgroundColor: 'var(--line-green)', padding: '6px 10px', fontSize: '0.8em', marginRight: '5px' }} onClick={() => handleSaveMachineClick(machine.id)}>บันทึก</button>
                              <button className="line-button" style={{ backgroundColor: '#6c757d', padding: '6px 10px', fontSize: '0.8em' }} onClick={handleCancelMachineEdit}>ยกเลิก</button>
                            </>
                          ) : (
                            <button className="line-button" style={{ backgroundColor: 'var(--primary-pink)', padding: '6px 10px', fontSize: '0.8em' }} onClick={() => handleEditMachineClick(machine)}>แก้ไข</button>
                          )}
                        </td>
                      </tr>
                    ))}
          ...
          {addingNewMachine ? (
            <form onSubmit={handleSaveNewMachine} style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxWidth: '350px', margin: '0 auto', fontSize: '0.9em', padding: '15px', border: '1px solid #eee', borderRadius: '8px' }}>
              ...
              <input type="number" placeholder="เช่น 1, 5, 10" value={newMachineFormData.machine_id} onChange={(e) => setNewMachineFormData({ ...newMachineFormData, machine_id: e.target.value })} style={{ padding: '8px', borderRadius: '5px', border: '1px solid #ccc' }}/>
              <label style={{ textAlign: 'left', fontWeight: 'bold' }}>ประเภท:</label>
              <select value={newMachineFormData.machine_type} onChange={(e) => setNewMachineFormData({ ...newMachineFormData, machine_type: e.target.value as 'washer' | 'dryer' })} style={{ padding: '8px', borderRadius: '5px', border: '1px solid #ccc' }}>
              ...
              <input type="number" placeholder="เช่น 25, 40" value={newMachineFormData.duration_minutes}
              ...
              <input type="text" placeholder="เช่น เครื่องซักผ้า #5" value={newMachineFormData.display_name} onChange={(e) => setNewMachineFormData({ ...newMachineFormData, display_name: e.target.value })} style={{ padding: '8px', borderRadius: '5px', border: '1px solid #ccc' }}/>
              <label style={{ textAlign: 'left', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
                <input type="checkbox" checked={newMachineFormData.is_active} onChange={(e) => setNewMachineFormData({ ...newMachineFormData, is_active: e.target.checked })} style={{ transform: 'scale(1.2)', marginRight: '8px' }}/>
                เปิดใช้งาน
              </label>
              <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '15px' }}>
                <button type="submit" className="line-button" style={{ backgroundColor: 'var(--line-green)', padding: '8px 15px', fontSize: '0.9em' }}>บันทึกเครื่องใหม่</button>
                <button type="button" className="line-button" style={{ backgroundColor: '#6c757d', padding: '8px 15px', fontSize: '0.9em' }} onClick={handleCancelNewMachine}>ยกเลิก</button>
              </div>
            </form>
          ) : (
            <button className="line-button" style={{ backgroundColor: 'var(--primary-pink)', padding: '10px 20px', fontSize: '1em' }} onClick={handleAddMachineClick}>
              เพิ่มเครื่องใหม่
            </button>
          )}
              ...
              {activeTimers.length === 0 ? (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px', fontSize: '0.9em' }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid var(--light-pink)' }}>
                      <th style={{ padding: '8px', textAlign: 'left', color: 'var(--dark-pink)' }}>เครื่อง</th>
                      <th style={{ padding: '8px', textAlign: 'left', color: 'var(--dark-pink)' }}>โดย</th>
                      <th style={{ padding: '8px', textAlign: 'left', color: 'var(--dark-pink)' }}>เสร็จสิ้น</th>
                      <th style={{ padding: '8px', textAlign: 'right', color: 'var(--dark-pink)' }}>จัดการ</th>
                    </tr>
                  </thead>
                  <tbody>
                    {activeTimers.map(timer => (
                      <tr key={timer.id} style={{ borderBottom: '1px dashed #eee' }}>
                        <td style={{ padding: '8px', fontWeight: 'bold' }}>{timer.display_name}</td>
                        <td style={{ padding: '8px', fontSize: '0.8em', fontFamily: 'monospace' }}>{timer.user_id.substring(4, 12)}...</td>
                        <td style={{ padding: '8px' }}>{new Date(timer.end_time.seconds * 1000).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })} น.</td>
                        <td style={{ padding: '8px', textAlign: 'right' }}>
                          <button className="line-button" style={{ backgroundColor: '#dc3545', padding: '6px 10px', fontSize: '0.8em' }} onClick={() => handleCancelTimer(timer.id, timer.display_name)}>
                            ยกเลิก
                          </button>
                        </td>
                      </tr>
          ...
          {loadingMessages ? (
            ...
            <div className="message-templates-list" style={{ textAlign: 'left', overflowX: 'auto' }}>
              {messageTemplates.length === 0 ? (
                <p style={{ textAlign: 'center', color: '#777', fontSize: '0.9em' }}>ไม่พบข้อมูลข้อความ</p>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px', fontSize: '0.9em' }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid var(--light-pink)' }}>
                      <th style={{ padding: '8px', textAlign: 'left', color: 'var(--dark-pink)' }}>ประเภท (ID)</th>
                  ...
                  <tbody>
                    {messageTemplates.map(template => (
                      <tr key={template.docId} style={{ borderBottom: '1px dashed #eee' }}>
                        <td style={{ padding: '8px', fontWeight: 'bold', fontFamily: 'monospace' }}>{template.id}</td>
                        <td style={{ padding: '8px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                          {editingMessageId === template.docId ? (
                            <textarea value={editMessageFormData} onChange={(e) => setEditMessageFormData(e.target.value)} rows={4} style={{ width: '100%', padding: '4px', borderRadius: '4px', border: '1px solid #ccc', resize: 'vertical' }} />
                          ) : (
                            template.text
                          )}
                        </td>
                        <td style={{ padding: '8px', textAlign: 'right' }}>
                          {editingMessageId === template.docId ? (
                            <>
                              <button className="line-button" style={{ backgroundColor: 'var(--line-green)', padding: '6px 10px', fontSize: '0.8em', marginRight: '5px' }} onClick={() => handleSaveMessageClick(template.docId)}>บันทึก</button>
                              <button className="line-button" style={{ backgroundColor: '#6c757d', padding: '6px 10px', fontSize: '0.8em' }} onClick={handleCancelMessageEdit}>ยกเลิก</button>
                            </>
                          ) : (
                            <button className="line-button" style={{ backgroundColor: 'var(--primary-pink)', padding: '6px 10px', fontSize: '0.8em' }} onClick={() => handleEditMessageClick(template)}>แก้ไข</button>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      <div className="card">
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <input
            ...
            style={{ padding: '10px', margin: '10px 0', borderRadius: '6px', border: '1px solid #ddd', width: '90%', maxWidth: '250px', fontSize: '1em' }}
          />
          {error && <p style={{ color: '#dc3545', fontSize: '0.9em', marginBottom: '10px' }}>{error}</p>}
          <button type="submit" className="line-button" style={{ backgroundColor: 'var(--primary-pink)', padding: '10px 20px', fontSize: '1em' }}>
            เข้าสู่ระบบ
          </button>
        </form>
      </div>
